...
// Bundle data: 10mm = 51.87kg, 12mm = 53.35kg
...
const FootingScanner = () => {
  ...
  const calculateBBS = (sizeFt: number, dia: number, spacing: number, count: number) => {
    ...
    // Formula: (Size + 0.6666) * NoBars / 3.281
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-gray-50 min-h-screen">
      <div className="bg-green-700 text-white p-8 rounded-t-2xl shadow-lg">
        <h1 className="text-3xl font-black italic">UNIQ DESIGNS - BBS SCANNER</h1>
      </div>
      <div className="bg-white p-8 border shadow-md rounded-b-2xl">
        <input type="file" onChange={handleFileUpload} className="block w-full text-sm mb-6" />
        {loading && <p className="text-center font-bold text-green-600 animate-pulse">Scanning Drawing...</p>}
        {results.length > 0 && (
            ...
            <tbody>
              {results.map((r, i) => (
                <tr key={i} className="text-center border-b">
                  <td className="p-3 font-bold border">{r.tag}</td>
                  <td className="p-3 border font-bold text-blue-600">{r.count}</td>
                  <td className="p-3 font-black text-red-600 border">{r.totalKg.toFixed(1)}</td>
                  <td className="p-3 border">{r.bundlesNeeded.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
